#!/usr/bin/env bash
# ============================================================
# MbemNova · Script 08/16 · Paiements Apprenant
# ============================================================
# Contenu :
#   payment.component.ts  (S07 · S08 · S16 · S17 · S18)
#     · Historique paiements + tranches
#     · Statuts colorés (reçu / partiel / retard / annulé)
#     · Demande de moratoire (S17) — modal
#     · Compte suspendu (S18) — message humain + CTA
...
mkdir -p src/app/features/learner/payment

echo -e "\n${B}══════════════════════════════════════════${N}"
echo -e "${B}  MbemNova · 08 · Paiements Apprenant     ${N}"
echo -e "${B}══════════════════════════════════════════${N}\n"

sec "payment.component.ts (S07 S08 S16 S17 S18)"

cat > src/app/features/learner/payment/payment.component.ts << 'EOF'
import {
...
import type {
  ...
  StatutPaiement, MoratoireRequest,
} from '../../../core/models';
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      <p class="text-xs text-slate-400">
                        @if (t.estPayee && t.datePaiement) {
                          ...
                          Échéance : {{ t.echeance | date:'dd MMMM yyyy':'':'fr' }}
                        }
                      </p>
                    </div>
...
export class PaymentComponent implements OnInit {
  ...
  submitMoratoire(): void {
    ...
    const req: MoratoireRequest = {
      paiementId:            id,
      raison:                raison as MoratoireRequest['raison'],
...
ok "payment.component.ts"

echo ""
echo -e "${G}══════════════════════════════════════════════${N}"
echo -e "${G}  Script 08 terminé ✓                         ${N}"
echo -e "${G}══════════════════════════════════════════════${N}"
echo ""
echo -e "  ${G}✓${N}  payment.component.ts"
echo -e "       · Historique paiements + échéancier tranches"
echo -e "       · Badges statut colorés (reçu/partiel/retard)"
echo -e "       · Barre progression paiement partiel"
echo -e "       · Indicateurs tranches : payée / en retard / bientôt / à venir"
echo -e "       · Compte suspendu (S18) — message humain + WhatsApp"
echo -e "       · Modal moratoire (S17) — raison + explication + date"
echo -e "       · Validation reactive form + erreurs inline"
echo -e "       · Skeleton chargement + empty state illustré"
echo -e "       · Bloc info paiement cash + lien WhatsApp"
echo ""
echo -e "  ${Y}→ Prochaine étape : ./ng09_learner_sessions.sh${N}"
echo ""
